...
CellActor::AdjustDownSlope(CellMap* _map){
    ...
    if(was_grounded == true && is_grounded == false && velocity.y > 0.0f){
        position.y = std::floor(position.y);
        if(
            (HeightUntilGround(_map, mask_decal, solid_layer, position, olc::RED) < snap_to_ground) ||
            (HeightUntilGround(_map, mask_decal, solid_layer, position) < snap_to_ground)
        ){
            while(
                !IsOverlapping(_map, mask_decal, solid_layer, position, olc::RED) ||
                !IsOverlapping(_map, mask_decal, solid_layer, position)
            ){
